...
NET_PATTERN_DIR = join(pwd, dpath, 'mobility-networks')
if not exists(NET_PATTERN_DIR):
	os.mkdir(NET_PATTERN_DIR)

NETWORK_SAVE_DIR = join(NET_PATTERN_DIR, f'{AREA_NAME}-CBG-CBG-Nets-msa')
...
for pattern_file in tqdm(pattern_files):
	...
	create_network(pattern_file, set(), save_dir=NETWORK_SAVE_DIR)
